...
opts = hlp_varargin2struct(varargin, ...
    ...
    {'wndlen','WindowLength'}, 10, ...          % window length to display in seconds
    {'newcol','NewColor'}, [0 0 0.5], ...       % color of the new (i.e., cleaned) data
    {'oldcol','OldColor'}, [1 0 0], ...         % color of the old (i.e., uncleaned) data
    {'highpass_old','HighpassOldData'},false, ...% whether to high-pass the old data if not already done
    {'show_removed_portions','ShowRemovedPortions'},false, ...% whether to show removed data portions (if only one set is passed in)
...
if opts.show_removed_portions
	...
	if length(size(new.data)) == 3
		new.data = new.data(:,:);
	end
	if length(size(old.data)) == 3
		old.data = old.data(:,:);
	end
	if length(size(new.data)) ~= 2 || length(size(old.data)) ~= 2
...
hSlider.SliderStep(1) = opts.wndlen * new.srate/length(new.data);
hSlider.SliderStep(2) = hSlider.SliderStep(1)*10;
jSlider = findjobj(hSlider);
jSlider.AdjustmentValueChangedCallback = @on_update;

annotation(hFig,'textbox', [0 1 1 0],...
    'String', {'[n/o/b/d] new/old/both/diff data [+/-] increase/decrease amp scale [*//] shrink/expand time scale [h] show/hide slider'},...
...
on_update();
    ...
    function repaint(hSlider, relPos,moved)
        ...
        hSlider.SliderStep(1) = wndsamples/max(length(new.data),length(old.data));
				hSlider.SliderStep(2) = hSlider.SliderStep(1)*10;
    ...
    function on_update(varargin)
        % slider moved
        repaint(hSlider, get(hSlider,'Value'),~isempty(varargin));
    end
...
function map = gen_colormap(eventstruct,mapname)
	...
	if isscalar(map.keys)
			eval(['c = ',mapname,';'])
			map.values = c(round(end/2),:);
	elseif ~isempty(map.keys)
			eval(['c = ',mapname,';'])
			map.values = c(1+floor((0:length(map.keys)-1)/(length(map.keys)-1)*(length(c)-1)),:);
	else
			map.values = [];
	end
end
