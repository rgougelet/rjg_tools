classdef iirsos
	...
	methods (Static)
		%% highpass method
		function [data] = hp(data,srate, sixdbcutoff_hz, passband_hz, desired_passband_ripple, plot_freq_response)
			tic;
		...
		%% bandstop method
		function [data] = bs(data,srate, inner_hz, outer_hz, desired_passband_ripple, plot_freq_response)
			tic
			if size(data,1)>size(data,2)
				warning('Only works on data with channels as rows');
			end
			...
			% 			disp(['IIR SOS Stopband: -6 dB cutoff point at: ',num2str(db_cutoff),' Hz'])
			
			% check pb ripple
			% 			lf = f <= outer_hz(1);
			% 			rf = outer_hz(2) <= f;
			% 			pbr = ~~(lf+rf);
			% 			disp(['IIR SOS Stopband: Average passband ripple: ',num2str(mean(abs(mag2db(abs(h(pbr)))))),' dB'])
			...
			x = flip(sosfilt(sos,flip(x)));
			data = reshape(x',size(data));
			disp(['IIR SOS Stopband: Filtered in ',num2str(toc,3),' seconds'])
		end
		
		%% bandpass method
		% inverts bandstop filter by swapping arguments
		function [data] = bp(data,srate, inner_hz, outer_hz, desired_passband_ripple, plot_freq_response)
			tic
			if size(data,1)>size(data,2)
				warning('Only works on data with channels as rows');
			end
			...
			[~,cutoff_i] = mink(abs(mag2db(abs(h))+6),2);
			db_cutoff = f(cutoff_i)';
			disp(['IIR SOS Passband: -6 dB cutoff point at: ',num2str(db_cutoff),' Hz'])
			
			% check pb ripple
			lf = f >= inner_hz(1);
			rf = inner_hz(2) >= f;
			pbr = (lf&rf);
			disp(['IIR SOS Passband: Average passband ripple: ',num2str(mean(abs(mag2db(abs(h(pbr)))))),' dB'])
			...
			mean(abs(x1-x_sosff))
			figure; plot(x1-x_sosff)
			
			x_abcdf = sosfilt(sos_abcd,x);
			x_abcdf = flip(sosfilt(sos_abcd,flip(x_abcdf)));
			mean(abs(x1-x_abcdf))
			figure; plot(x1-x_abcdf)

			x_zpkf = sosfilt(sos_zpk,x);
			x_zpkf = flip(sosfilt(sos_zpk,flip(x_zpkf)));
			mean(abs(x1-x_zpkf))
			figure; plot(x1-x_zpkf)
			
			x_abcd = filtfilt(sos_abcd,G_abcd,x);
			mean(abs(x1-x_abcd))
			figure; plot(x1-x_abcd)
			
			x_zpk = filtfilt(sos_zpk,G_zpk,x);
			mean(abs(x1-x_zpk))
			figure; plot(x1-x_zpk)

			data = reshape(x',size(data));
			disp(['IIR SOS Passband: Filtered in ',num2str(toc,3),' seconds'])
		end
	end
end

%% trash
% 			pass_left = passband_hz(1)./nyq ;
% 			pass_right= passband_hz(2)./nyq;
% 			stop_left = stopband_hz(1)./nyq ;
% 			stop_right= stopband_hz(2)./nyq;