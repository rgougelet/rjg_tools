classdef iirsos
  ...
  methods (Static)
    ...
    function [...
        ...
        if verbose
          % check -6db cutoff
          disp(['IIR SOS Bandpass: Intended -6dB cutoff points: ', sprintf('%4.2f %4.2f', outer_hz)])
          [db_at_cutoff, cutoff_i] = mink(abs(mag2db(abs(h))+6), 2);
          [cutoff_i, sort_i] = sort(cutoff_i);
          db_at_cutoff = db_at_cutoff(sort_i);
          db_at_cutoff = db_at_cutoff-6;
          disp(['IIR SOS Bandpass: -6 dB cutoff points at: ', ...
            num2str(f(cutoff_i)','%4.2f %4.2f'),' Hz']);
...
%       xpad = [zeros(pn, 1); x'; zeros(pn,1)];
      xpad = [zeros(1,srate*5), x, zeros(1,srate*5)];

      % iir
      tic;
      iir_data = iirsos.bp(xpad, srate, [5 6], [4.9,7.1], .1, 1, 1);
